// Precio de una reserva. UNICA fuente de verdad: la usan la pantalla de
// reserva, el resumen de pago y el servidor de pagos (api/stripe), que
// recalcula el importe en vez de creerle al telefono.
//
// Sin imports de React Native a proposito: este archivo se empaqueta tambien
// en la funcion serverless de Vercel.
//
// Todo se calcula en CENTAVOS enteros. Con decimales flotantes salian
// importes como 255.525 MXN, que Braintree rechaza y que la app mostraba tal
// cual. Los pesos solo aparecen al final, redondeados a 2 decimales.
...
// "$1,234.50" — mismo formato en toda la app.
export function formatMXN(amount: number | undefined | null): string {
  const value = Number.isFinite(amount as number) ? (amount as number) : 0;
  return mxnFormatter ? mxnFormatter.format(value) : `$${value.toFixed(2)}`;
}
