...
export function createSeed() {
  ...
  rows.forEach(([status, guardId, day, duration, vehicleType], i) => {
    ...
    const scheduled = new Date(status === 'active' ? Date.now() - 30 * 60000 : iso(day));
    ...
    const b = { id, clientId: 'client', guardId, companyId: 'company', status, bookingType: 'scheduled', vehicleType, protectionType: 'unarmed', dressCode: 'suit', numberOfProtectees: 1, numberOfProtectors: 1, scheduledDate: `${scheduled.getFullYear()}-${String(scheduled.getMonth()+1).padStart(2,'0')}-${String(scheduled.getDate()).padStart(2,'0')}`, scheduledTime: `${String(scheduled.getHours()).padStart(2,'0')}:${String(scheduled.getMinutes()).padStart(2,'0')}`, duration, pickupAddress: 'Quinta Avenida y Calle 12, Playa del Carmen', pickupCity: 'Playa del Carmen', pickupLatitude: 20.6269, pickupLongitude: -87.073, hourlyRate: users[guardId].hourlyRate, totalAmount: subtotal + processingFee, processingFee, platformCut: price.platformCut, guardPayout: price.guardPayout, createdAt: iso(day - 3), ...(status === 'pending' ? {} : { confirmedAt: iso(day - 3), transactionId: `demo-${id}`, paymentIntentId: `demo-${id}` }), ...(['accepted', 'active', 'completed'].includes(status) ? { acceptedAt: iso(day - 2) } : {}), ...(['active', 'completed'].includes(status) ? { startedAt: scheduled.toISOString() } : {}), ...(status === 'completed' ? { completedAt: iso(day, 23) } : {}), ...(status === 'cancelled' ? { cancelledAt: iso(day - 1), cancelledBy: 'client', cancellationReason: 'Plans changed' } : {}) };
    if (i === 4) { b.rating = 5; b.review = 'Punctual and discreet. Sample review.'; docs[`reviews/${id}`] = { guardId, bookingId: id, clientId: 'client', rating: 5, ratingBreakdown: { professionalism: 5, punctuality: 5, communication: 5, languageClarity: 5 }, review: b.review, createdAt: b.completedAt }; }
    root.bookings[id] = b;
